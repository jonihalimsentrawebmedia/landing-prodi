...
const CurriculumPageV11 = (props: Props) => {
  const { data } = props
  ...
  return (
    <>
      <JumbotronTitleV11 title={'Kurikulum'} context={'KURIKULUM'} />
      ...
      <div className="bg-footer/10 py-5">
        <div className="py-2.5 container-sm">
          ...
          <Tabs
            className={'w-full h-full mt-5 flex-col! gap-5'}
            value={year}
            onValueChange={(v) => {
              const Params = new URLSearchParams(searchParams.toString())
              Params.set('year', v)
          ...
          >
            <TabsList
              className={clsx(
                'w-full h-full flex-col! bg-transparent',
                'lg:flex-row! justify-start overflow-x-scroll lg:overflow-x-visible',
                'flex-row!'
              )}
            >
              {elements?.map((row, k) => (
                <TabsTrigger
                  ...
                  value={row?.value}
                  className={cn(
                    'shadow-none rounded-none w-full border-b-2 border-b-primary p-2',
...
export default CurriculumPageV11
