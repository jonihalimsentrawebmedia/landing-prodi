...
const CurriculumPageV8 = (props: Props) => {
  const { data } = props
  ...
  return (
    <>
      <JumbotronTitleV8 title={'Kurikulum'} context={'KURIKULUM'} />
      ...
      <div className="bg-footer">
        <div className="py-2.5 container-sm">
          ...
          >
            <TabsList className={'h-full bg-footer gap-3 lg:w-fit w-full overflow-x-scroll lg:overflow-x-visible justify-start'}>
              {elements?.map((row, k) => (
                <TabsTrigger
                  key={k}
                  value={row?.value}
                  className={cn(
                    'shadow-none border rounded border-primary',
...
export default CurriculumPageV8
